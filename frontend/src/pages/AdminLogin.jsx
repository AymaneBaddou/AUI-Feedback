  ...
  const handleMicrosoftLogin = async () => {
    ...
    try {
      ...
      if (!ALLOWED_ADMIN_EMAILS.includes(email)) {
        setError(
          `The account ${rawEmail} is not allowed to access the admin dashboard.`
        );
        setLoading(false);
        return;
      }
      ...
      console.error("Admin Microsoft login error:", err);
      // Show a clearer message if it's the backend, but keep generic on UI
      setError("Microsoft login failed. Please try again.");
    } finally {
      setLoading(false);
    }
